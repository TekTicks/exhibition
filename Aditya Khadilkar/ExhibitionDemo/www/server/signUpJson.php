...
else
{
	...
	$selectVisitorQuery1 = "select * from visitor where mobileNumber='$mobileNo'";
	...
	else
	{
		...
		$selectVisitorQuery2="select * from visitor where email='$email'";
		...
		else
		{
			...
			$insertVisitorQuery="insert into visitor (name,mobileNumber,email,password,createdOn,modifiedOn)  values('$name','$mobileNo','$email','$password','$createdOn','$createdOn')";
			$insertVisitor=mysql_query($insertVisitorQuery,$conn) or die(mysql_error());
			//select id from comment table
			$selectVisitorQuery1 = "select id from visitor where mobileNumber='$mobileno'";
			$selectVisitor1 = mysql_query($selectVisitorQuery1 ,$conn) or die(mysql_error());
